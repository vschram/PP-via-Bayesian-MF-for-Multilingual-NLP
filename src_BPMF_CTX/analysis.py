...
def analyse_rmse_mcmc(logger, other_params, mf, train, test, fmtest, fmtrain, fold, run, i, path, alpha, flag_test=0):
    predicted, results = mf.running_rmse(fmtest, fmtrain, np.array(test), np.array(train), run, i, fold, path, other_params[2], other_params[1], alpha, flag_test = flag_test, burn_in=other_params[11])
