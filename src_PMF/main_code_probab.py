...
import random
from utils_probab import parse_args
import socket
from helpers import generate_score_matrix, get_rmse, get_permutations, get_median_values, create_directory, get_data, create_dict_for_results
from get_information import get_info, get_context, get_filename
from split_functions import split_k_fold_data, split_k_fold_data_ncv, split_k_fold_data_nlperf
from logger import create_logger
from PMF import PMF
from analysis import analyse_rmse_mcmc, analyse_summary, analyse_mcmc


def get_result(filename, path, logger, other_params, dim, alpha, beta, data_dir, scores, src_index, tgt_index, k, num_running, std, src_lang_side_info=None,
    ...
    if other_params[0]:
        ...
        logger.info(f'ALPHA (precision) = {alpha}, BETA = [var_w = {beta[0]}, var_h = {beta[1]}, var_x = {beta[2]}, var_y = {beta[3]}, var_z = {beta[4]}]')
    ...
    # BETA = [reg_w, reg_h, reg_x, reg_y, reg_z]
    run_alpha_vec, run_var_w_vec, run_var_h_vec, run_var_x_vec, run_var_y_vec, run_var_z_vec, = [], [], [], [], [], []
    ...
    for nr in range(num_running):
        ...
        model_alpha_vec, model_var_w_vec, model_var_h_vec, model_var_x_vec, model_var_y_vec, model_var_z_vec = [], [], [], [], [], []

        for model in scores:

            if other_params[3] == 'NCV' or other_params[3] == 'NCV_NLPerf':
                if other_params[0]:
                    logger.info('*' * 20)
                ...
                # -----------------------------------------------------
                # BETA = [reg_w, reg_h, reg_x, reg_y, reg_z]
                alpha_vec, var_w_vec, var_h_vec, var_x_vec, var_y_vec, var_z_vec = [], [], [], [], [], []
                ...
                for i in range(k):
                        ...
                        for hypset in range(len(hyperparam_settings)):
                            ...
                            for ii in range(k-1):
                                ...
                                dev_matrix = generate_score_matrix(dev_data, src_index, tgt_index, model,
                                                                     langs_matrix)
                                # BETA = [var_w, var_h, var_x, var_y, var_z] train, flag, var, dim=2, alpha=2, std=0.01
                                mf = PMF(model, logger, other_params, train_matrix, beta, dim=dim, alpha=alpha, std=std)

                                # Get train, dev RMSEs
                                # ------------------------------
                                #other_params = [params.enable_log, params.doplots, params.savepngs, split, task,
                                                #attribute, ctx, eval_on_dev,
                                                #params.chains, params.tune, params.draw, params.burn]
                                ...
                                predicted, results, cur_rmse_dev, _ = analyse_rmse_mcmc(logger, other_params, mf, train_matrix, dev_matrix, ii, nr, i, path, alpha, beta, 0)
                    ...
                    train_matrix_full = generate_score_matrix(train_data_complete, src_index, tgt_index, model,
                                                              langs_matrix, 1, i+1, other_params[2])
                    test_matrix = generate_score_matrix(test_data, src_index, tgt_index, model,
                                                       langs_matrix, 2, i+1, other_params[2])

                    mf_full = PMF(model, logger, other_params, train_matrix_full, argmin_h_dev_error['beta'], dim=dim, alpha=argmin_h_dev_error['alpha'], std=std)
                    trace = mf_full.draw_samples(cores=1, draws=other_params[10], tune=other_params[9],
                                            chains=other_params[8],
                                            progressbar=True, return_inferencedata=False)

                    predicted, results, cur_rmse, cur_rmse_train = analyse_rmse_mcmc(logger, other_params, mf_full,
                                                                                     train_matrix_full, test_matrix, None, nr, i, path, alpha, beta, 1)
                    ...
                    alpha_vec.append(argmin_h_dev_error['alpha']), var_w_vec.append(argmin_h_dev_error['beta'][0]), var_h_vec.append(argmin_h_dev_error['beta'][1])
                    var_x_vec.append(argmin_h_dev_error['beta'][2]), var_y_vec.append(argmin_h_dev_error['beta'][3]), var_z_vec.append(argmin_h_dev_error['beta'][4])
                ...
                # -----------------------------------------
                median_alpha, median_var_w, median_var_h, median_var_x, median_var_y, \
                median_var_z = get_median_values(alpha_vec, var_w_vec, var_h_vec, var_x_vec, var_y_vec, var_z_vec)
            ...
            # BETA = [reg_w, reg_h, reg_x, reg_y, reg_z, reg_bias_s, reg_bias_t]
            model_alpha_vec.append(median_alpha), model_var_w_vec.append(median_var_w), model_var_h_vec.append(median_var_h)
            model_var_x_vec.append(median_var_x), model_var_y_vec.append(median_var_y), model_var_z_vec.append(median_var_z)
        ...
        # -----------------------------------------
        median_alpha, median_var_w, median_var_h, \
        median_var_x, median_var_y, median_var_z = get_median_values(model_alpha_vec, model_var_w_vec, model_var_h_vec,\
                                                                     model_var_x_vec, model_var_y_vec, model_var_z_vec)
        # -----------------------------------------
        # Average per model here
        # -----------------------------------------
        ...
        # -----------------------------------------------------
        # BETA = [reg_w, reg_h, reg_x, reg_y, reg_z, reg_bias_s, reg_bias_t]
        run_alpha_vec.append(median_alpha), run_var_w_vec.append(median_var_w), run_var_h_vec.append(median_var_h)
        run_var_x_vec.append(median_var_x), run_var_y_vec.append(median_var_y), run_var_z_vec.append(median_var_z)

    # -----------------------------------------
    # find median hyperparam setting aver all runs
    # -----------------------------------------
    run_median_alpha, run_median_var_w, run_median_var_h, \
    run_median_var_x, run_median_var_y, run_median_var_z = get_median_values(run_alpha_vec, run_var_w_vec, run_var_h_vec, \
                                                                             run_var_x_vec, run_var_y_vec, run_var_z_vec)
    ...
    final_result = [running_rmse, running_dev_rmse, running_train_rmse, run_median_alpha, \
        [run_median_var_w, run_median_var_h, run_median_var_x, run_median_var_y, \
         run_median_var_z]]
    ...
    if other_params[0]:
        ...
        logger.info(f'median_beta run: {[run_median_var_w, run_median_var_h,run_median_var_x, run_median_var_y, run_median_var_z]}')
        logger.info('#################################')
...
if __name__ == '__main__':
    ...
    if other_params[0]:
        ...
        if attribute == "si":
            ctx = None
        logger = create_logger(f"{pathlogs}/Probab_{filename}.log")
        logger.info('#' * 100)
    ...
    result = get_result(filename, path, logger, other_params, DIM, ALPHA, BETA, DIR, SCORE, SRC, TGT, FOLDS, num_running, STD,\
