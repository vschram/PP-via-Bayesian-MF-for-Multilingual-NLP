...
def analyse_rmse_mcmc(logger, other_params, mf, train, test, fold, run, i, path, alpha, beta, flag_test=0):
    predicted, results = mf.running_rmse(np.array(test), np.array(train), run, i, fold, path, other_params[2], other_params[1], alpha, beta, flag_test = flag_test, burn_in=other_params[11])

    # -----------------------
    # Dev RMSE
    # -----------------------
    if flag_test == 0:
        test_rmse = results["running-dev"].values[-1]
        ...
        if other_params[0]:
            ...
            logger.info("Posterior predictive dev RMSE:  %.5f" % test_rmse)
            logger.info("Train/dev difference:           %.5f" % (test_rmse - train_rmse))
            logger.info('-' * 20)
    else:
