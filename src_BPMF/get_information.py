...
def get_info(task, attributes, ctx, params, DIR):
    ...
    return SRC, TGT, SCORE, SIDE_FEATURES, CONTEXT_FEATURES, tgt_si_len, src_si_len, \
           lang_pair_si_len, SIDE_INFO_DICT, SRC_SIDE_INFO_DICT, TGT_SIDE_INFO_DICT, ctx, attributes
...
def get_context(context, task):
    ...
    if task == 'wiki_aaMF':
        ...
        ctx = ['Rank', 'Accuracy level', 'Transfer lang dataset size', 'Transfer over target size ratio',
               'Transfer lang TTR', 'Target lang TTR', 'Transfer target TTR distance', 'GENETIC', 'SYNTACTIC', 'GEOGRAPHIC']

    elif task == 'tsfparsing_aaMF':
        ctx = ['Rank', 'Accuracy level', 'Transfer lang dataset size', 'Target lang dataset size', 'Transfer over target size ratio',
               'Transfer lang TTR', 'Target lang TTR', 'Transfer target TTR distance', 'GENETIC', 'SYNTACTIC', 'GEOGRAPHIC']

    elif task == 'tsfmt_aaMF':
        ctx = ['Rank', 'BLEU level', 'Overlap word-level', 'Overlap subword-level', 'Transfer lang dataset size', 'Target lang dataset size',
               'Transfer over target size ratio', 'Transfer lang TTR', 'Target lang TTR', 'Transfer target TTR distance', 'GENETIC',
               'GENETIC_2', 'SYNTACTIC_2', 'FEATURAL_2', 'PHONOLOGICAL_2', 'INVENTORY_2', 'GEOGRAPHIC_2']

    elif task == 'tsfel_aaMF':
        ctx = ['Rank', 'Accuracy level', 'Transfer lang dataset size', 'Target lang dataset size', 'Transfer over target size ratio', 'GENETIC',
            'SYNTACTIC', 'FEATURAL', 'PHONOLOGICAL', 'GEOGRAPHIC']
